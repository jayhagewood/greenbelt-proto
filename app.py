...
# Sources: Census ACS (B05003B etc.), Census TIGERweb (GeoJSON), PG County Socrata
# Designed for Streamlit Cloud with lazy imports
...
BOUNDARY_SOURCE = st.sidebar.selectbox(
    "Boundary source",
    ["Census Places (TIGER)", "OSM (Nominatim)"],
    index=0,
)
...
def get_city_boundary_from_census_places(place_name: str, state_fips: str):
    ...
    Works reliably on Streamlit Cloud (no vsizip issues).
    """
    gpd = _gpd()
    base = (
    ...
    name_token = place_name.split(",")[0].strip().replace("'", "''")
...
if city_gdf is not None:
    ...
    with colB:
        st.subheader("Notes")
        st.info(
            "ADOS-proxy = Native-born Black (alone) from ACS table B05003B. "
            "This is a proxy measure, not a full ancestry definition."
        )
else:
    st.warning("No boundary — cannot compute ADOS proxy")

st.markdown("---")
st.caption("Prototype — Sources: Census ACS 5-year, TIGERweb GeoJSON, OSM (optional).")
